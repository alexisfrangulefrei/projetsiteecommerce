...
exports.handler = async (event) => {
  console.log("[Lambda] SQS order-processing lambda triggered with event:", JSON.stringify(event));
  if (event.Records && Array.isArray(event.Records)) {
    ...
    for (const record of event.Records) {
      try {
        ...
        console.log(`[Lambda] Stock status for order ${orderId}: ${status}`);
        // 1. Save in DynamoDB (OrderDB)
        console.log(`[Lambda] Saving order ${orderId} to OrderDB...`);
        ...
        // 2. Archive in DynamoDB (ArchiveDB)
        console.log(`[Lambda] Archiving order ${orderId} to ArchiveDB...`);
        await dynamo.send(new PutItemCommand({
          TableName: 'ArchiveDB',
          Item: {
            orderId: { S: orderId },
        ...
        if (status === 'valid') {
          ...
          console.log(`[Lambda] Generating invoice for order ${orderId} in S3...`);
        ...
        // 4. Send email via SES (simulation)
        let emailBody;
        if (status === 'valid') {
        ...
        console.log(`[Lambda] Sending email for order ${orderId} to ${email}...`);
    ...
    console.error('[Lambda] Invalid event format:', event);
